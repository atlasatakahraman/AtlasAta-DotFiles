...
|---------|--------|----------|
| `:w` | Save file | Command — Ctrl+S is intentionally unbound |
...
|---------|-----------|-------|
| `Escape` / `Ctrl+[` | Insert → Normal | Standard Escape |
...
| Keybind | Action |
|---------|--------|
| `Leader+ur` | Relative line numbers on/off |
| `Leader+uw` | Line wrap on/off |
| `Leader+ul` | Whitespace markers on/off |
| `Leader+ud` | Inline diagnostics on/off |
| `Leader+uz` | Zen mode — hide tabline, statusline, numbers, signcolumn |
| `Leader+uc` | Colorscheme picker (Telescope) |
| `Leader+th` | Theme light/dark toggle |
| `Leader+ci` | LSP inlay hints on/off (buffer) |
...
| `Alt+1-5` | Go to buffer 1-5 | Matches the number shown on the tab |
...
| Inside Neo-tree: `H` | Toggle hidden files |
| Inside Neo-tree: `I` | Toggle git-ignored files |
...
- **`Escape` key**: Exit Insert mode back to Normal mode (or `<C-[>`)
...
### Mode Switching Customization

Insert mode uses standard `Escape` or `Ctrl+[`. If you want a custom sequence like `jk`, you can edit `lua/keymaps/editing.lua`:
```lua
