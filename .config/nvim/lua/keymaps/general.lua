...
-- ── Save / Quit ───────────────────────────────────────────────
-- No <C-s>: save with :w. 'confirm' is on (see core/options.lua) so :q on a
-- dirty buffer asks instead of refusing.
...
-- NOTE: <C-x> (cut) lives in keymaps/editing.lua for all modes — don't duplicate it here.
