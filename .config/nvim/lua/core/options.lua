...
opt.smoothscroll = true             -- Scroll by screen line, not buffer line (wrapped text)
opt.splitkeep = "screen"            -- Text stays put when a split opens above it
opt.pumblend = 10                   -- Slight translucency on the completion popup
opt.winblend = 0                    -- Floats stay opaque (readability over flash)

-- NOT setting opt.winborder: telescope/plenary draw their border as a separate
-- floating window, so a global winborder gives them two nested borders. Every
-- float in this config already sets its own rounded border (blink, noice,
-- which-key, mason, neo-tree, toggleterm, diagnostics).

-- Cleaner glyphs: no "~" past the last line, thin split separators
opt.fillchars = { eob = " ", diff = "╱" }

-- Show the whitespace that actually causes bugs, hide the rest
opt.list = true
...
opt.timeoutlen = 500                -- Time in ms to wait for mapped key sequences
...
opt.confirm = true                  -- Prompt to save on :q instead of erroring out
